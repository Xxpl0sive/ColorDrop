...
async function zephiidrop() {
  ...
  makeTriggerBox(new BS.Vector3(-2,-308.9,-4), new BS.Vector3(90,1,90), new BS.Vector4(0,0,0,0), () => {
      if(isStarted) { isStarted = false; duration = new Date().getTime() - startTime; checkSpaceState(zephiiscene.localUser.name.replace(/<[^>]*>/g, '').replace(/["'\\]/g, '\\$&'), duration); setPublicSpaceProp("latestjump:" + zephiiscene.localUser.name.replace(/<[^>]*>/g, '').replace(/["'\\]/g, '\\$&'), duration);}
  });
